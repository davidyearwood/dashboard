...
export default class Signup extends React.Component {
    ...
    // set hasErrors to true and prevent user from submiting 
    // otherwise allow the user to submit
    handleUserFormSubmit(e) {
        let data = {

        }; 
        if(this.state.email.hasError || this.state.firstName.hasError || this.state.lastName.hasError) {
            e.preventDefault();
        } else {
            e.preventDefault(); 
